...
def load_models(gdino_checkpoint="/home/non/nhan_ws/src/SmartWorkcell/io/weights/groundingdino_swinb_cogcoor.pth",
    ...
    """Load & Return GroundingDINO and NanoSAM models."""
    # load gdino
    gdino_model = Model(
...
def load_camera_intrinsics(file_path="/home/non/nhan_ws/src/SmartWorkcell/io/config/camera_intrinsics.yml"):
    with open(file_path) as f:
        config = yaml.safe_load(f)
    intrinsics = config["realsense1"]
    return intrinsics

def realsense_setup():
    """Initialize & Return RealSense pipeline, config, and colorizer.
    
    Returns
    -------
    pipeline: rs.pipeline
        RealSense pipeline for streaming data.
    config: rs.config
        Configuration for the RealSense pipeline.
    colorizer: rs.colorizer
        Colorizer to visualize depth data.
    """
    pipeline = rs.pipeline()
    config = rs.config()
    colorizer = rs.colorizer()
